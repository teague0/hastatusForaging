...
load("./data/HastatusGPS_HMMRelevelstates.Rdata")

#One set of values for eps & minpts doesn't really work for all of the tracks. Looks like the break into eps = 5, 10, or 20. Plot diagnostic plots to help sort this out.
...
#Assign custom EPS based on the diagnostics from amov
ids <- names(hast_L)
...
#save(clus, file="./data/DBSCAN cluster ID.Rdata")
...
clusFrgFlw <- mclapply(X=clus, mc.cores = detectCores()-2, FUN = function(x){
  tryCatch({
    ...
    tmp <- lapply(wlist, function(newClus){
      ...
      xy2event_id <- newClus %>% filter(newState %in% c("state2")) %>% dplyr::select(event_id)
      xy2$event_id <- xy2event_id$event_id
      xy3event_id <- newClus %>% filter(newState %in% c("state3")) %>% dplyr::select(event_id)
      xy3$event_id <- xy3event_id$event_id
      xy4event_id <- newClus %>% filter(newState %in% c("state4")) %>% dplyr::select(event_id)
      xy4$event_id <- xy4event_id$event_id
      xy2$dbClus_feed2 <- NA
      xy3$dbClus_feed3 <- NA
      xy4$dbClus_feed4 <- NA
    ...
    clusterVals <- tmpdat %>% dplyr::select(event_id, dbClus_feed2, dbClus_feed3, dbClus_feed4)
    dat <- left_join(x, clusterVals, by="event_id")
    return(dat)
  },error=function(e) NULL)
  }) 
#save(clusFrgFlw, file="./data/Clustering_ForagSites.Rdata")

hastAll_df <- do.call("rbind", newStates)
forage_df <- do.call("rbind", clusFrgFlw)
clusters <- forage_df %>% dplyr::select(event_id, dbClus:dbClus_feed4)
hastClass_df <- left_join(hastAll_df, clusters)
#save(hastClass_df, file="./ata/HastatusHMMdbClus_flw.Rdata")
